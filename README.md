# Library-Management
#### This is a Python Project for a Book Store data using MySQL where we have given the features which are reqired in a Library and working on the data is easy and user friendly 


App Screens
-------
| Home Screen    |
| ------------- |
| <img src ="p1.jpg" width = "900" height = "550">     |
|<h4> This is the home page which comes at the starting giving diffrent options in the Book Store  |
  
<br/>
  
| Add Book Details |
| ------------- |
| <img src ="p2.jpg" width = "900" height = "550">      |
|<h4> You can add a new book by providing with Book title and author's Name  |

<br/>
  
| Delete Book Details     |
| ------------- |
| <img src ="p3.jpg" width = "900" height = "550">      |
|<h4> You can delete a book by providing with Book ID  |

<br/>
  
| View Book Details       |
| ------------- |
| <img src ="p4.jpg" width = "900" height = "550">      |
|<h4> Here you can see all the book list which are Avail and Issued  |
  
<br/>
  
| Issue Book |
| ------------- |
| <img src ="p5.jpg" width = "900" height = "550">      |
|<h4> You can issue the avialable books by providing Book ID and Issued to name  |

<br/>
  
| Return Book     |
| ------------- |
| <img src ="p6.jpg" width = "900" height = "550">      |
|<h4> You can return a book by providing with Book ID  |
  
  
 ## Libraries Used 📖
- Tkinter
- Pillow
- mysql.connector
