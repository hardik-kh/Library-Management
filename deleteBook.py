...
def issuebook():
    ...
    for i in cr:
        if(str(bid)==str(i[0])):
            print(bid)
            check=1
            cr.execute('delete from final where id ='+str(bid))
...
def delete():
  ...
  #  cr = cn.cursor(buffered=True)
    ...
    headingLabel = Label(headingFrame1, text="Delete Books", bg='black', fg='white', font=('Courier',22,'bold'))
    ...
    lb1.place(relx=0.2,rely=0.8)
        
    inf1 = Entry(root1,font=('Arial'))
    inf1.place(relx=0.3,rely=0.8, relwidth=0.42,relheight=0.04)
    
        #Issue Button
    issueBtn = Button(root1,text="Delete",bg='white', fg='black',command=issuebook)
